...
describe('MatchScreen live commentary order', () => {
  ...
  it('renders latest commentary before crease and field controls can push it off screen', () => {
    const commentaryIndex = source.indexOf('accessibilityLabel={`Latest commentary');
    const partnershipIndex = source.indexOf('{/* Partnership Tracker */}');
    const fieldIndex = source.indexOf('<View style={styles.midRow}>');
