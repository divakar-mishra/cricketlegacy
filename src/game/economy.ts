...
export function canSpendEnergy(wallet: Wallet, amount: number): boolean {
  const cost = Math.floor(amount);
  return cost > 0 && wallet.energy >= cost;
}
...
export interface NoSpendLoopOptions {
  minutes: number;
  matchDurationMinutes?: number;
  startingWallet?: Wallet;
}

export interface NoSpendLoopResult {
...
export interface FirstWeekNoSpendOptions {
  days?: number;
...
export interface FirstWeekNoSpendResult {
  days: number;
...
/**
 * Deterministic no-spend play loop model. It assumes the player starts a match
 * whenever they have enough energy, each match consumes real time, and no ads,
 * IAPs or premium boosts are used. This is not a replacement for real playtest
 * feedback; it is a guardrail against obvious energy-wall regressions.
 */
export function simulateNoSpendLoop(options: NoSpendLoopOptions): NoSpendLoopResult {
  ...
  while (now < totalMinutes) {
    ...
    const waitMinutes = missing * ECONOMY.energyRegenMinutes;
    now += Math.max(1, waitMinutes);
  }
...
/**
 * First-week no-spend model. The player opens the game once per day, plays
 * greedily for a fixed session length, and relies only on natural energy regen.
 */
export function simulateFirstWeekNoSpendLoop(
  options: FirstWeekNoSpendOptions = {},
): FirstWeekNoSpendResult {
  ...
  for (let day = 0; day < days; day++) {
    ...
    while (now < sessionEnd) {
      ...
      energyBlocks++;
      blockedToday = true;
      const missing = cost - wallet.energy;
      const waitMinutes = missing * ECONOMY.energyRegenMinutes;
      now += Math.max(1, waitMinutes);
    }
  ...
  const weekEnd = days * 24 * 60 * 60 * 1000;
  wallet = regenEnergy(wallet, weekEnd);
